...
public class ActivityController {
	...
	public void delete(@PathVariable Long id, HttpServletRequest req) {
		...
		if (author != null) {
			...
			if (toDelete.getAuthor().equals(author))  {
	...
	@PutMapping("{id}")
	public Activity update(@PathVariable Long id, @RequestBody Activity input, HttpServletRequest req) {
		User author = findUser(req.getCookies());
		if (author != null) {
			Activity activity = activityRepository.findOne(id);
			if (activity == null || !activity.getAuthor().equals(author)) {
				return null;
			} else {
	...
	public List<Activity> filter(@PathVariable String name) {
		...
		List<Activity> filtered = activities.stream()
				.filter(activity -> activity.getTags().stream()
